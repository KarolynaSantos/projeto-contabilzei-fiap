...
--Utilize nomes significativos e coerentes, de acordo com a base do Correio. Uma sugestão de link para acesso seria:  https://buscacepinter.correios.com.br/app/endereco/index.php
...
VALUES (SQ_MC_CIDADE.NEXTVAL, 'SP', 'São Paulo', 123, 11);
...
INSERT INTO MC_BAIRRO(CD_BAIRRO, CD_CIDADE, NM_BAIRRO, NM_ZONA_BAIRRO) 
VALUES (SQ_MC_BAIRRO.NEXTVAL, 3 ,'Icaraí', 'Zona Oseste');

INSERT INTO MC_BAIRRO(CD_BAIRRO, CD_CIDADE, NM_BAIRRO, NM_ZONA_BAIRRO) 
VALUES (SQ_MC_BAIRRO.NEXTVAL, 4 ,'Cambuí', 'Zona Leste');
...
INSERT INTO MC_LOGRADOURO(CD_LOGRADOURO, CD_BAIRRO, NM_LOGRADOURO, NR_CEP) 
VALUES (SQ_MC_LOGRADOURO.NEXTVAL, 2, 'Avenida Atlântica', 22010000);

INSERT INTO MC_LOGRADOURO(CD_LOGRADOURO, CD_BAIRRO, NM_LOGRADOURO, NR_CEP) 
VALUES (SQ_MC_LOGRADOURO.NEXTVAL, 2, 'Rua Barata Ribeiro', 22011002);

INSERT INTO MC_LOGRADOURO(CD_LOGRADOURO, CD_BAIRRO, NM_LOGRADOURO, NR_CEP) 
VALUES (SQ_MC_LOGRADOURO.NEXTVAL, 3, 'Rua Gavião Peixoto', 24230091);

INSERT INTO MC_LOGRADOURO(CD_LOGRADOURO, CD_BAIRRO, NM_LOGRADOURO, NR_CEP) 
VALUES (SQ_MC_LOGRADOURO.NEXTVAL, 3, 'Avenida Sete de Setembro', 28400000);

INSERT INTO MC_LOGRADOURO(CD_LOGRADOURO, CD_BAIRRO, NM_LOGRADOURO, NR_CEP) 
VALUES (SQ_MC_LOGRADOURO.NEXTVAL, 4, 'Avenida Júlio de Mesquita', 06070156);

INSERT INTO MC_LOGRADOURO(CD_LOGRADOURO, CD_BAIRRO, NM_LOGRADOURO, NR_CEP) 
VALUES (SQ_MC_LOGRADOURO.NEXTVAL, 4, 'Rua Coronel Quirino', 13025004);


-- d) Por fim, cadastre na tabela de ENDERECO FUNCIONARIO os endereços de 2 funcionários à sua escolha. Diversifique os dados escolhendo cidades e estados diferentes. 
--Caso deseje, você poderá cadastrar mais de um endereço por funcionário.
...
VALUES (SQ_MC_CLIENTE.NEXTVAL, 'João da Silva', 2, 120, 'A', 'joao@gmail.com', 11959826578, 's.joão', 1489);
...
-- Exiba a instrução SQL executada para realizar a tarefa e apresente o resultado dessa execução).  Foi possível incluir esse novo cliente?  Explique?
...
-- Não é possivel incluir o novo cliente, pois decorre da quebra de uma restrição exclusiva (unique constraint) denominada "UK_MC_CLIENTE_MM_LOGIN" na tabela "MC_CLIENTE". 
--Essa situação ocorre quando se tenta adicionar um novo cliente com o mesmo valor no campo "NM_LOGIN" que já existe na tabela para outro cliente.
-- A restrição exclusiva tem o propósito de assegurar que não ocorram duplicatas no campo "NM_LOGIN", todos os valores neste campo devem ser únicos. 
--Visto que já existe um cliente com o mesmo nome de login registrado no banco de dados, a tentativa de incluir um novo cliente com o mesmo "NM_LOGIN" infringe essa restrição, levando à exibição do erro ORA-00001.

-- Inserindo dados para cadastro de pessoa física
...
-- Inserindo dados para cadastro de pessoa jurídica
...
-- Inserindo dados para endereço cliente
...
VALUES (SQ_MC_CATEGORIA.NEXTVAL, 'P', 'Produtos Eletrônicos',TO_DATE('19042023','DDMMYYYY'), null, 'A');
...
INSERT INTO MC_PRODUTO(CD_PRODUTO, CD_CATEGORIA, NR_CD_BARRAS_PROD, DS_PRODUTO, VL_UNITARIO, TP_EMBALAGEM, ST_PRODUTO, VL_PERC_LUCRO, DS_COMPLETA_PROD) 
VALUES (SQ_MC_PRODUTO.NEXTVAL, 3, 0152795831477, 'Ração para Cães', 30, 'Pacote', 'A', 0.15, 'Ração de alta qualidade.');

INSERT INTO MC_PRODUTO(CD_PRODUTO, CD_CATEGORIA, NR_CD_BARRAS_PROD, DS_PRODUTO, VL_UNITARIO, TP_EMBALAGEM, ST_PRODUTO, VL_PERC_LUCRO, DS_COMPLETA_PROD) 
VALUES (SQ_MC_PRODUTO.NEXTVAL, 1, 0152795831478, 'Fone de Ouvido', 200, 'Unidade', 'A', 0.10, 'Fones de ouvido premium.');

INSERT INTO MC_PRODUTO(CD_PRODUTO, CD_CATEGORIA, NR_CD_BARRAS_PROD, DS_PRODUTO, VL_UNITARIO, TP_EMBALAGEM, ST_PRODUTO, VL_PERC_LUCRO, DS_COMPLETA_PROD) 
VALUES (SQ_MC_PRODUTO.NEXTVAL, 1, 0152795831479, 'Mouse Sem Fio', 80, 'Unidade', 'A', 0.15, 'Mouse de alta precisão.');
...
INSERT INTO MC_CATEGORIA_PROD(CD_CATEGORIA, TP_CATEGORIA, DS_CATEGORIA, DT_INICIO, DT_TERMINO, ST_CATEGORIA) 
VALUES (SQ_MC_CATEGORIA.NEXTVAL, 'V', 'Instalação do produto',TO_DATE('12082023','DDMMYYYY'), null, 'A');
...
VALUES (1, SQ_MC_SGV_SAC.NEXTVAL, 1 , null, 'Padrão', 'Video como usar', 'A');

INSERT INTO MC_SGV_PRODUTO_VIDEO(CD_PRODUTO, NR_SEQUENCIA, CD_CATEGORIA, VD_PRODUTO, TP_VIDEO_PROD, ds_path_video_prod, st_video_prod) 
VALUES (1, SQ_MC_SGV_SAC.NEXTVAL, 2 , null, 'Padrão', 'Video como usar', 'A');
...
--Não é possível excluir o estado 'SP' da tabela 'MC_ESTADO' porque existem restrições de integridade referencial ou dependências em outras tabelas que estão vinculadas a este estado. 
--O erro 'ORA-02292' ocorre quando uma tentativa de exclusão viola a integridade referencial, o que significa que há registros filhos na tabela 'MC_CIDADE' que dependem do estado 'SP'. 
--Essas restrições de integridade existem para garantir a consistência dos dados e evitar a remoção acidental de registros que estão sendo referenciados em outras partes do banco de dados.
...
--O erro 'ORA-02290' ocorre porque a tentativa de atualização do status do produto para 'X' na tabela 'MC_PRODUTO' viola uma restrição de verificação (CHECK constraint) chamada 'MC_PRODUTO_CK_ST_PROD'. 
--Esta restrição de verificação foi definida para garantir que apenas valores específicos sejam permitidos no campo 'ST_PRODUTO'. Como 'X' não é um valor permitido pela restrição de verificação, 
--a tentativa de atualização viola essa regra, resultando no erro. Essas restrições de verificação são usadas para garantir a integridade dos dados e manter a consistência das informações na tabela.
